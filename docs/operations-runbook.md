...
正常解析结果继续按微批次入库。若 COPY/约束等数据错误导致事务失败，worker 的共享
持久化服务会在回滚并关闭该次会话后，二分未完成文件，用新事务重试，直到隔离出单个失败文件。
重试复用内存中的 MolOP 解析结果，不重新读取/上传原始文件，也不重复清理旧解析结果。
已提交窗口的逐文件结果会保留，不重复生成解析版本；处理租约在重试期间继续校验，
单文件最终失败由 worker 原有完成流程记录。

日志 `splitting failed persistence microbatch` 表示拆分重试，
`isolated artifact persistence failure` 才表示已定位单文件失败。
数据库断连、超时、死锁及资源类错误不进行逐文件扇出重试；取消操作继续走原有恢复流程。
此机制不修复源文件字符或修改 MolOP 输出，也不会自动重试此前已终结的失败任务。
