...
import { ArrowRight, ArrowUpRight, CircleAlert } from "@lucide/vue";
...
const detailRoute = computed(() => ({
  ...
  params: { logicalReactionId: props.reaction.id },
...
<template>
  <article ref="cardRoot" class="reaction-path-card">
    <header class="reaction-card-header">
      <div>
        <span class="eyebrow">LogicalReaction · {{ reaction.cycloaddition_pattern || labelFor(reaction.reaction_class) }}</span>
    ...
    <footer class="reaction-card-footer">
      ...
      <span v-if="reaction.similarity_score !== null" class="reaction-similarity">映射反应相似度 {{ (reaction.similarity_score * 100).toFixed(1) }}%</span>
      ...
      <span v-else class="reaction-barrier is-missing">暂无 ΔG</span>
      <span class="reaction-mapping-count">{{ reaction.mapped_reaction_count ?? "—" }} 个映射反应</span>
      <RouterLink class="reaction-card-detail-link" :to="detailRoute">打开逻辑反应详情 <ArrowUpRight :size="14" aria-hidden="true" /></RouterLink>
      <code>{{ reaction.reaction_key }}</code>
    </footer>
    <RouterLink class="reaction-card-direct-link" :to="detailRoute" title="在独立页面打开" :aria-label="`在独立页面打开反应路径 ${reaction.id}`"><ArrowUpRight :size="16" aria-hidden="true" /></RouterLink>
  </article>
</template>
