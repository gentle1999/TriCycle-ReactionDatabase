...
<template>
  <main class="entity-detail-page reaction-detail-page" aria-labelledby="reaction-detail-title">
    ...
    <section v-else class="reaction-detail-body">
      <div class="reaction-detail-summary"><span>{{ reactionQuery.data.value.reaction_class }}</span><code :title="reactionQuery.data.value.id">{{ reactionQuery.data.value.id }}</code><span>{{ reactionQuery.data.value.participants.length }} 个参与物 · {{ reactionQuery.data.value.mapped_reactions.length }} 个映射方案</span></div>
